...
(
		...
		CONSTRAINT NIVELES CHECK(NIVEL IN('JARDIN DE NIÑOS','PRIMARIA','SECUNDARIA','PREPARATORIA'))
);
...
--1ERA PARTE: SECUENCIAS
CREATE SEQUENCE AUTO_IDSERVICIO
START WITH 10000000 INCREMENT BY 1;


--2DA PARTE: ALTAS
CREATE OR REPLACE PROCEDURE PROCALTASRVCIO(VTIPO IN VARCHAR2,VPERIODOPAGO IN NUMBER)
AS
...
EXEC PROCALTASRVCIO('COLEGIATURA',12);
EXEC PROCALTASRVCIO('INSCRIPCION',0);
EXEC PROCALTASRVCIO('MATERIAL ESCOLAR',1);
EXEC PROCALTASRVCIO('COLEGIATURA',12);--NO PUEDE SER REPETIDO Y ATRAPA LA EXCEPCIÓN
EXEC PROCALTASRVCIO('UNIFORME',1);
--DELETE FROM SERVICIO;

--3RA PARTE: BAJAS

CREATE OR REPLACE PROCEDURE PROCBAJASRVCIO(VIDSERVICIO IN NUMBER)
AS
BEGIN
...
EXEC PROCBAJASRVCIO(10000002);
EXEC PROCBAJASRVCIO(10000099);--ÉSTE REGISTRO NO EXISTE, SE ATRAPA LA EXCEPCIÓN

--4TA PARTE: CAMBIOS
CREATE OR REPLACE PROCEDURE PROCCAMBIOSRVCIO(VIDSERVICIO IN NUMBER,
	VTIPO IN VARCHAR2,VPERIODOPAGO IN NUMBER)
AS
BEGIN
	...
	ELSE
		DBMS_OUTPUT.PUT_LINE('SE ACTUALIZÓ UN SERVICIO');
		COMMIT;
	END IF;
END PROCCAMBIOSRVCIO;
/

--DATOS DE PRUEBA
SELECT * FROM SERVICIO;

EXEC PROCCAMBIOSRVCIO(10000001,'LIBROS',1);
EXEC PROCCAMBIOSRVCIO(10000099,'BUTACAS',1);--ÉSTE SERVICIO NO EXISTE, SE ATRAPA LA EXCEPCIÓN
